...
export function VacancyListVacancyStatusControl({
  presentationKey,
  vacancyStatus
}: {
  ...
  return <td className="px-4 py-3"><InlineSelect label="Изменить статус вакансии" value={vacancyStatus} pending={update.isPending} onChange={(value) => update.mutate({ presentationKey, payload: { vacancy_status: value as VacancyStatus } })} options={statuses.map((status) => ({ value: status, label: vacancyStatusLabels[status] }))} />{update.isError && <p role="alert" className="mt-1 text-xs text-rose-700">Не сохранено</p>}</td>;
}

export function VacancyListUserPriorityControl({
  presentationKey,
  userPriority
}: {
  presentationKey: string;
  ...
  return <td className="px-4 py-3"><InlineSelect label="Изменить мой приоритет" value={userPriority ?? ""} pending={update.isPending} onChange={(value) => update.mutate({ presentationKey, payload: { user_priority: (value || null) as VacancyUserPriority | null } })} options={priorities.map((priority) => ({ value: priority, label: priority || "Не оценено" }))} />{update.isError && <p role="alert" className="mt-1 text-xs text-rose-700">Не сохранено</p>}</td>;
}

export function VacancyListUserCommentControl({
  presentationKey,
  comment
}: {
  presentationKey: string;
