...
import { ManualVacancyCrmSyncStatus } from "../components/ManualVacancyCrmSyncStatus";
...
export function VacancyDetailPage() {
  ...
  return <div className="space-y-6">
    <header className="space-y-4">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-sm font-medium text-zinc-500">{vacancy.company}</p><h1 className="mt-1 max-w-4xl text-2xl font-semibold leading-tight">{vacancy.title}</h1></div>{vacancy.url && <a href={vacancy.url} target="_blank" rel="noopener noreferrer" className="inline-flex shrink-0 items-center gap-2 bg-zinc-900 px-4 py-2.5 text-sm font-medium text-white hover:bg-zinc-700"><ExternalLink size={17} />Открыть оригинал</a>}</div>
    </header>

    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
      <main className="min-w-0 space-y-6"><VacancyDescription description={vacancy.description} /><ApplicationSection applications={vacancy.applications} vacancyId={vacancy.vacancy_id} /><VacancyUserStateSection state={vacancy.user_state} />{vacancy.manual_crm_sync && <section className="border border-line bg-white p-5"><h2 className="font-semibold">Google Sheets CRM</h2><div className="mt-3"><ManualVacancyCrmSyncStatus sync={vacancy.manual_crm_sync} /></div></section>}
        {vacancy.member_count > 1 && <section className="border border-line bg-white p-5"><h2 className="text-lg font-semibold">Региональные копии: {vacancy.member_count}</h2><ul className="mt-4 divide-y divide-line">{vacancy.members.map((member) => <li key={`${member.source}:${member.external_id}`} className="flex flex-wrap items-center justify-between gap-3 py-3 first:pt-0 last:pb-0"><div><p className="font-medium">{member.location ?? "Локация не указана"}{member.representative && <span className="ml-2 bg-zinc-900 px-2 py-0.5 text-xs font-semibold text-white">Представитель</span>}</p><p className="mt-1 font-mono text-xs text-zinc-500">{member.source}:{member.external_id}</p></div>{member.url && <a href={member.url} target="_blank" rel="noopener noreferrer" className="text-sm font-medium text-zinc-700 underline decoration-zinc-300 underline-offset-4 hover:decoration-zinc-900">Открыть</a>}</li>)}</ul></section>}
