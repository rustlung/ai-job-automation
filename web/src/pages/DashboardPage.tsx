...
import { LegacyCrmKeyMigrationStatus } from "../components/LegacyCrmKeyMigrationStatus";
...
export function DashboardPage() {
  ...
  return <div className="space-y-8">
    ...
    <section><div className="mb-3 flex items-center justify-between"><h2 className="text-base font-semibold">Последний запуск</h2><Link to="/runs" className="inline-flex items-center gap-1 text-sm font-medium text-zinc-700 hover:text-zinc-950">Открыть список <ArrowRight size={15} /></Link></div>{runs.isLoading ? <LoadingState /> : runs.isError ? <ErrorState /> : !latestRun ? <EmptyState title="Запусков пока нет" detail="Выберите поисковые профили и начните первый запуск." /> : <div className="space-y-3"><Link to={`/runs/${latestRun.run_id}`} className="block border border-line bg-white p-5 shadow-sm transition hover:border-zinc-400"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-mono text-sm text-zinc-600">{latestRun.run_id}</p><p className="mt-2 text-sm text-zinc-500">{formatTriggerSource(latestRun.trigger_source)} · {formatDateTime(latestRun.started_at)}</p></div><RunStatusBadge status={latestRun.status} /></div><p className="mt-4 text-sm text-zinc-700">{latestRun.profile_ids.join(", ") || "Профили не указаны"}</p></Link><LegacyCrmKeyMigrationStatus matches={latestRun.stats_snapshot?.legacy_crm_key_matches} /></div>}</section>
  </div>;
}
